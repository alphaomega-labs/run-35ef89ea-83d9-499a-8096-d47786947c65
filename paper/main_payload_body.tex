...
The executed validation used three image families with fixed splits and rank sweeps at $k\in\{8,16,24,32,48\}$. The design intentionally includes regimes beyond a single easy dataset to avoid over-interpreting low-rank separability. While the current execution used locally available proxy variants rather than freshly downloaded canonical manifests, the split protocol, rank grid, and comparator parity controls were held fixed throughout. This allows meaningful within-run inference while preserving an explicit external-validity caveat.
...
\Figref{fig:did_eta} and Table~\ref{tab:cf2} evaluate whether entanglement contributes positive causal utility after controlling for observable policy. The DiD estimator in \eqref{eq:did} is the relevant object because it removes additive main effects. In the executed run, the overall mean interaction was slightly negative, and positive slices were outnumbered by non-positive slices. Runtime-penalized utility in \eqref{eq:utility} also declined as entanglement strength increased.
...
\renewcommand{\arraystretch}{1.1}
\setlength{\tabcolsep}{4pt}
\resizebox{\linewidth}{!}{\input{tables/tab_hm_cf2_factorial_effects.tex}}
\end{table}
...
The interpretation is intentionally scoped. The bound audit uses surrogate error components and fixed-regularization assumptions, so it does not imply a universal equivalence theorem for all adaptive settings. Instead, it provides a practically auditable boundary in the current regime and a concrete target for future stress tests.

\begin{figure}[t]
\centering
\includegraphics[width=0.68\linewidth]{figures/fig_hm_cf3_null_region_heatmap.pdf}
...
\textbf{Approximations.} The current iteration used proxy dataset variants and surrogate channel-degradation stress rather than full channelized retraining. These approximations are explicitly carried as caveats in main-text interpretation.
